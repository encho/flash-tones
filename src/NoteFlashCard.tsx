...
function NoteFlashCard({ note, type, isActive = false }: NoteFlashCardProps) {
  ...
  const clamped  = cents !== null ? Math.max(-DISPLAY_RANGE, Math.min(DISPLAY_RANGE, cents)) : 0;
  const position = clamped / DISPLAY_RANGE;

  const needleColor =
    cents === null           ? '#9ca3af'
    : Math.abs(cents) < MATCH_CENTS ? '#22c55e'
    : Math.abs(cents) < 40  ? '#eab308'
    :                          '#ef4444';
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: '0.85rem', color: '#888', letterSpacing: '0.1em' }}>
        {type}
      </span>
      <button
